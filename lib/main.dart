...
void main() {
  ...
  ui.platformViewRegistry.registerViewFactory('camera-view', (int viewId) {
...
class _EarringOverlayState extends State<EarringOverlay> {
  Offset leftEarringPos = const Offset(180, 200);
  Offset rightEarringPos = const Offset(330, 200);
  double earringSize = 70;
  int selectedEarring = 0;

  final earrings = ['assets/earrings/ear1.png', 'assets/earrings/ear2.png'];

  /// Take snapshot of current preview (with earrings)
  void _takeSnapshot() {
    final canvas = html.CanvasElement(width: 640, height: 480);
    final ctx = canvas.context2D;

    final video = html.document.querySelector('video');
    if (video != null && widget.liveMode) {
      ctx.translate(canvas.width!.toDouble(), 0);
      ctx.scale(-1, 1); // mirror flip for selfie
      ctx.drawImage(video as html.CanvasImageSource, 0, 0);
    }

    // Add earrings
    final img1 = html.ImageElement(src: earrings[selectedEarring]);
    final img2 = html.ImageElement(src: earrings[selectedEarring]);

    img1.onLoad.listen((_) {
      ctx.drawImageScaled(img1, leftEarringPos.dx, leftEarringPos.dy,
          earringSize, earringSize);
      ctx.drawImageScaled(img2, rightEarringPos.dx, rightEarringPos.dy,
          earringSize, earringSize);

      final dataUrl = canvas.toDataUrl('image/png');
      final anchor = html.AnchorElement(href: dataUrl)
        ..download = 'earring_tryon.png'
        ..click();
    });
  }
                  ...
                  children: List.generate(earrings.length, (i) {
                    return GestureDetector(
                      ...
                      child: AnimatedContainer(
                        ...
                        child:
                            Image.asset(earrings[i], width: 50, height: 50),
                      ),
                    );
                  }),
                ),
